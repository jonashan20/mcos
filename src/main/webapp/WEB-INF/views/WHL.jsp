...
<form  action = "SWHL" method = "POST">
<input type="hidden" id="userid" name="userid" value="20201230"/><br/>
<div>
	<div style="border:1px solid #dedede; width:205px; height:70px; line-height:30px; color:#666;font-size:30px; text-align:center;" id="clock"></div>
		<div class="row-col-6">				
 			<input type="time" id="startHour" name="startHour" pattern="([1]?[0-9]|2[0-3]):[0-5][0-9]"/><br/>
 		 	<input class="col-2" type="submit" value="출근"   /><br/>
 		 	<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" />  
		 </div>
 	</div>
</form> 	
<form  action = "EWHL" method = "POST">
<input type="text" id="userid" name="userid" value="${userid}"/><br/>
<div>
		<div class="row-col-6">		
 			<input type="time" id="endHour" name="endHour" pattern="([1]?[0-9]|2[0-3]):[0-5][0-9]"/>
			&nbsp;&nbsp;유형
			 <select id="D_DOH" name="type">
			 <option value="0">정상</option>
              <option value="1">반차</option>          
           </select> 
           <br>
			<input class="col-2" type="submit" value="퇴근" /><br/>
			<input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}" /> 
